...
def test_get_products(test_client, admin_auth_headers):
    ...
    response = test_client.get("/api/estudiantes", headers=admin_auth_headers)
...
def test_create_product(test_client, admin_auth_headers):
    ...
    response = test_client.post("/api/estudiantes", json=data, headers=admin_auth_headers)
    assert response.status_code == 201
    assert response.json["nombre"] == "Smartphone"
    assert response.json["edad"] == 2
    assert response.json["peso"] == 89.99
    assert response.json["bueno"] == True
...
def test_get_product(test_client, admin_auth_headers):
    ...
    response = test_client.get("/api/estudiantes/1", headers=admin_auth_headers)
    assert response.status_code == 200
...
def test_get_nonexistent_product(test_client, admin_auth_headers):
    ...
    response = test_client.get("/api/estudiantes/999", headers=admin_auth_headers)
...
def test_create_product_invalid_data(test_client, admin_auth_headers):
    ...
    data = {"nombre": "Laptop"}  # Falta description, price y stock
    response = test_client.post("/api/estudiantes", json=data, headers=admin_auth_headers)
...
def test_update_product(test_client, admin_auth_headers):
    ...
    response = test_client.put("/api/estudiantes/1", json=data, headers=admin_auth_headers)
    assert response.status_code == 200
    assert response.json["nombre"] == "Smartphone"
    assert response.json["edad"] == 2
    assert response.json["peso"] == 89.99
...
def test_update_nonexistent_product(test_client, admin_auth_headers):
    ...
    response = test_client.put(
        "/api/estudiantes/999", json=data, headers=admin_auth_headers
    )
    assert response.status_code == 404
    assert response.json["error"] == "Producto no encontrado"
...
def test_delete_product(test_client, admin_auth_headers):
    ...
    response = test_client.delete("/api/estudiantes/1", headers=admin_auth_headers)
...
def test_delete_nonexistent_product(test_client, admin_auth_headers):
    ...
    response = test_client.delete("/api/estudiantes/999", headers=admin_auth_headers)
    assert response.status_code == 404
    assert response.json["error"] == "Producto no encontrado"
