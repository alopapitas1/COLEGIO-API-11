def test_get_products_as_user(test_client, user_auth_headers):
    ...
    response = test_client.get("/api/estudiantes", headers=user_auth_headers)
...
def test_create_product_as_user(test_client, user_auth_headers):
    ...
    response = test_client.post("/api/estudiantes", json=data, headers=user_auth_headers)
...
def test_get_product_as_user(test_client, user_auth_headers):
    ...
    response = test_client.get("/api/estudiantes/1", headers=user_auth_headers)
    assert response.status_code == 200
    assert "nombre" in response.json
    assert "edad" in response.json
    assert "peso" in response.json
    assert "bueno" in response.json
...
def test_update_product_as_user(test_client, user_auth_headers):
    ...
    response = test_client.put("/api/estudiantes/1", json=data, headers=user_auth_headers)
    assert response.status_code == 403
...
def test_delete_product_as_user(test_client, user_auth_headers):
    ...
    response = test_client.delete("/api/estudiantes/1", headers=user_auth_headers)
    assert response.status_code == 403
